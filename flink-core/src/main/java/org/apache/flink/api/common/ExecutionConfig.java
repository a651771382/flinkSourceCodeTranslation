...
public class ExecutionConfig implements Serializable, Archiveable<ArchivedExecutionConfig> {
	 ...
	 * 指示使用默认并行度的标志值。此值可用于将并行性重置回默认状态。
	 */
	 ...
	 * unchanged.
	 * 标志值指示未知或未设置的并行度。 此值不是有效的并行性，表示并行性应保持不变。
	 */
	 ...
	 * 用于未指定最大并行度的运算符的程序范围最大并行度。 最大并行度指定动态缩放的上限和用于分区
	 * 状态的key的数量。
	 */
	 ...
	 * 不再使用，因为它已被RestartStrategyConfiguration包含
	 */
	@Deprecated
	 ...
	 * 如果设置为true，则在执行期间将进度更新打印到System.out
	 */
	 ...
	 * 超时后，正在进行的任务取消将导致致命的TaskManager错误，通常会杀死JVM
	 */
	 ...
	 * 此标志定义我们是否对状态快照数据使用压缩。 默认值：false
	 */
